<?php
// purchasing/reports.php
require_once __DIR__ . '/../config/database.php';
include_once __DIR__ . '/../layouts/header.php';
include_once __DIR__ . '/../layouts/sidebar.php';
include_once __DIR__ . '/../layouts/navbar.php';

// Proteksi Hak Akses: Hanya Staff Purchasing yang boleh melihat halaman ini
...
// Inisialisasi parameter filter rentang tanggal
$start_date = isset($_GET['start_date']) ? $_GET['start_date'] : '';
$end_date   = isset($_GET['end_date']) ? $_GET['end_date'] : '';

// LOGIKA QUERY DENGAN FILTER TANGGAL KHUSUS DATA STAFF INI
try {
    $query_str = "SELECT t.no_transaksi, t.tanggal_transaksi, pr.no_request, pr.jumlah, pr.total_harga, 
                         p.nama_barang, p.harga AS harga_satuan
                  FROM transactions t
    ...
    // Kondisional jika filter tanggal aktif digunakan oleh staff
    if (!empty($start_date) && !empty($end_date)) {
        $query_str .= " AND t.tanggal_transaksi BETWEEN :start_date AND :end_date";
    }
    
    $query_str .= " ORDER BY t.id_transaction DESC";
    $stmt = $pdo->prepare($query_str);
    
    // Penanganan parameter binding
    $params = ['id_user' => $id_user_staff];
    if (!empty($start_date) && !empty($end_date)) {
    ...
    // Hitung akumulasi total pengeluaran dana oleh staff ini
    $grand_total = 0;
    ...
    die("Gagal memuat laporan internal staff: " . $e->getMessage());
}
?>
...
<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 25px;">
    <div>
        <h1 style="font-size: 22px; font-weight: 700; color: #0f172a;">Laporan Pengadaan Saya</h1>
        <p style="color: #64748b; font-size: 13px; margin-top: 2px;">Rekapitulasi berkas realisasi pengadaan barang yang Anda ajukan dan telah disetujui.</p>
    </div>
...
<div id="print-area">
        ...
        <table class="modern-table">
            <thead>
                <tr>
                    ...
                    <th style="text-align: right;">Total Pagu Belanja</th>
                </tr>
            </thead>
            <tbody>
                <?php if (count($report_data) === 0): ?>
                    <tr>
                        <td colspan="7" style="text-align: center; color: #94a3b8; padding: 40px;">Tidak ada riwayat laporan pengadaan yang disetujui pada periode ini.</td>
                    </tr>
                <?php else: ?>
    ...
    <div class="summary-box">
        <span style="font-size: 14px; color: #64748b; font-weight: 500;">AKUMULASI PENYERAPAN ANGGARAN OLEH ANDA:</span>
        <h2 style="font-size: 24px; font-weight: 800; color: #2563eb; margin-top: 5px;">Rp <?= number_format($grand_total, 0, ',', '.'); ?></h2>
    </div>
</div>
