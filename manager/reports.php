...
// Proteksi Hak Akses: Hanya Manager yang boleh melihat halaman ini
if ($_SESSION['role'] !== 'Manager') {
    echo "<div style='padding:30px;'><h3>Akses Ditolak!</h3>Halaman ini hanya untuk tingkatan Manager.</div>";
...
// LOGIKA QUERY DENGAN FILTER TANGGAL GLOBAL UNTUK MANAGEMENT
try {
    ...
    // Kondisional jika filter tanggal aktif digunakan oleh Manager
    ...
    // Penanganan parameter binding jika filter aktif
    if (!empty($start_date) && !empty($end_date)) {
    ...
    die("Gagal memuat laporan eksekutif: " . $e->getMessage());
}
?>
...
<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 25px;">
    <div>
        <h1 style="font-size: 22px; font-weight: 700; color: #0f172a;">Laporan Pengadaan Finansial</h1>
        <p style="color: #64748b; font-size: 13px; margin-top: 2px;">Rekapitulasi berkas realisasi pengadaan barang korporasi yang telah divalidasi dan disetujui.</p>
    </div>
...
<div id="print-area">
        ...
        <table class="modern-table">
            ...
            <tbody>
                <?php if (count($report_data) === 0): ?>
                    <tr>
                        <td colspan="8" style="text-align: center; color: #94a3b8; padding: 40px;">Tidak ada riwayat pengadaan yang disetujui pada periode ini.</td>
                    </tr>
                <?php else: ?>
    ...
    <div class="summary-box">
        <span style="font-size: 14px; color: #64748b; font-weight: 500;">TOTAL REALISASI ANGGARAN OPERASIONAL:</span>
        <h2 style="font-size: 24px; font-weight: 800; color: #d97706; margin-top: 5px;">Rp <?= number_format($grand_total, 0, ',', '.'); ?></h2>
    </div>
</div>
